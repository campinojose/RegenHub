alter table public.consultas
  add column if not exists monto_medicamentos numeric not null default 0,
  add column if not exists estado_pago_medicamentos public.estado_pago not null default 'pendiente';

update public.consultas
set
  monto_medicamentos = greatest(coalesce(total_factura, 0) - coalesce(precio_consulta, 50000), 0),
  estado_pago_medicamentos = coalesce(estado_pago_medicamentos, estado_pago)
where monto_medicamentos = 0;
